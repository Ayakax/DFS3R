...
from model.backbones.ops.modules import MSDeformAttn
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, src_flatten, src_spatial_shapes, src_level_start_index, src_valid_ratios):
        B, C, N = src_flatten.shape[:3]
        # (B, C, N, 256) ---> (B*N, C, 256)
        band_flatten = src_flatten.permute(0, 2, 1, 3).contiguous().view(B * N, C, self.d_model)
        band_spatial_shapes = torch.tensor([1, C], device=src_flatten.device)[None, ...]
        band_valid_ratios = torch.ones([B * N, 1, 2], device=src_flatten.device)
        band_start_index = torch.tensor([0], device=src_flatten.device)

        # encoder
        memory = self.encoder(band_flatten, band_spatial_shapes, band_start_index, band_valid_ratios)  # (B*N, C, 256)
        memory = memory.view(B, N * C, self.d_model)  # (B, N*C, 256)

        # prepare cr/br input for decoder
        ...
        query_embed = query_embed.expand(B, -1, -1)  # (B, num_query, 256)
        tgt = tgt.expand(B, -1, -1)  # (B, num_query, 256)
        reference_points = self.reference_points(query_embed).sigmoid()  # (B, num_query, 2)

        hs, _, _ = self.decoder(tgt, reference_points, memory, src_spatial_shapes, src_level_start_index, src_valid_ratios, query_embed)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        ...
        # cross attention
        tgt2, sampled_points = self.cross_attn(self.with_pos_embed(tgt, query_pos),
                               reference_points,
